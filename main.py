...
async def on_ready():
  ...
  game = discord.Game("Pokémon in Beta")
  await bot.change_presence(status=discord.Status.online, activity=game)

#on message and image recog.
@bot.event
    ...
    if len(message.embeds) > 0:
      ...
      if "appeared!" in embed.title and embed.image:
        ...
        async with message.channel.typing():
          if name in rares and name != 'natu':
            embed = discord.Embed(color=None)
            embed.set_footer(text="❤️ The OutN Project")
            embed.add_field(name=None,
                            value='A rare Pokemon spawned! catch it using:')
            embed.add_field(name=None, value=f"@Pokétwo#8236 c {name}")
            ...
            embed.set_footer(text="❤️ The OutN Project")
            embed.add_field(name=None,
                            value='A Pokemon spawned! catch it using:')
            embed.add_field(name=None, value=f"@Pokétwo#8236 c {name}")
            await message.channel.send(embed=embed)
